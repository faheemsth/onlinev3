		...
		if(permissionChecker('certificatereport')) {
			if($_POST) {
				...
				if($this->form_validation->run() == FALSE) {
					...
					if($registerNO!='') {
						$studentArray['registerNO'] = $registerNO;
					}
	...
	public function generate_certificate() {
		...
		if(permissionChecker('certificatereport')) {
			if((int)$userID && (int)$usertypeID && (int)$templateID && (int)$schoolyearID && (int)$classID) {
				...
				$parents = $this->parents_m->get_single_parents(array('parentsID' => $student->parentID));
				...
				if(customCompute($student) && customCompute($usertype) && customCompute($template) && customCompute($schoolyear)) {
					...
					$tagArray['[father_name]'] 		= ucwords(strtolower($parents->name));
					$tagArray['[name]'] 		= ucwords(strtolower($student->srname));
					$tagArray['[dob]'] 			= isset($student->dob) ? date("d M Y", strtotime($student->dob)) : '';
					$tagArray['[gender]'] 		= $student->sex;
					$tagArray['[session]'] 		= $student->session_year;
					...
					$tagArray['[section]'] 		= ordinal(customCompute($tagSection) ? $tagSection->section : '');
					...
					$this->data['template'] = $this->tagConvertForTemplate($userTags, $template->template, 3, $tagArray,false);
