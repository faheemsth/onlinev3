...
class Studentrelation_m extends MY_Model {
	...
	private function userRelation() {
		...
		if($usertypeID == 2) {
			...
			$this->db->select('classesID')->from('subjectteacher')->where(array('teacherID' => $userID))->order_by('classesID');
    ...
    public function get_order_by_student_enrollment($arrays = [], $studentExtend = FALSE) {
		...
		$checkdate 	=	$arrays['checkdate'];
		unset($arrays['checkdate']);
		$arrays = $this->prefixLoad($arrays);
        $this->db->select('studentrelation.*,student.*,subjectenrollment.created as enroll_date');
        $this->db->from('studentrelation');
        $this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
        ...
        $this->db->group_start(); 
	    	$this->db->or_where('subjectenrollment.created IS NULL', NULL);    		 
	    	$this->db->or_where('date(subjectenrollment.created) >', date('Y-m-d',strtotime($checkdate)));    		 
	    $this->db->group_end(); 
	...
	public function get_student_with_balance($array=NULL){
		$this->db->select('studentrelation.*');
		$this->db->select('student.balance');
		$this->db->from('studentrelation');
        $this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
        $this->db->where('studentID IS NOT NULL');
        if(customCompute($array)) {
			$this->db->where($array);
		}
		$this->db->where('studentrelation.srsection !=', 11);
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function update_studentrelation_classes($data, $array = NULL) {
		$this->db->set($data);
		$this->db->where($array);
