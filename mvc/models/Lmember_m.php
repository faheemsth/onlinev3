...
class Lmember_m extends MY_Model {
	...
	public function get_all_librarybooks_for_report($queryArray) {
        ...
        if(isset($queryArray['classesID']) && $queryArray['classesID'] != 0) {
            $this->db->where('issue.usertypeID', 3);
        ...
        if((isset($queryArray['name']) && $queryArray['name'] != '')||(isset($queryArray['roll']) && $queryArray['roll'] != '')||(isset($queryArray['regno']) && $queryArray['regno'] != '')) {
             
             $this->db->where('issue.usertypeID', 3);
            $this->db->group_start();

            if (isset($queryArray['name']) && $queryArray['name'] != '') {
               $name    =   $queryArray['name'];
               $this->db->or_where('studentrelation.srname like', "%$name%");
            } 
            if (isset($queryArray['roll']) && $queryArray['roll'] != '') {
               $roll    =   $queryArray['roll'];
               $this->db->or_where('studentrelation.srroll like', "%$roll%");
            } 
            if (isset($queryArray['regno']) && $queryArray['regno'] != '') {
               $regno    =   $queryArray['regno'];
               $this->db->or_where('studentrelation.srregisterNO like', "%$regno%");
            }
    ...
    public function get_all_librarybooks_for_report_by_alluser($queryArray) {
        $currentdate = date('Y-m-d');
        $this->db->select('*');
        $this->db->from('lmember');
        $this->db->join('issue', 'issue.lID = lmember.lID');
